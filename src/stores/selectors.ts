...
// ---------------------------------------------------------------------------
// Shallow-equal helper (avoids re-render when extracted array hasn't changed)
// ---------------------------------------------------------------------------
...
function useStableArray<T>(
  ...
  const prev = useRef<T[]>([]);
  const next = selector();
  if (isEqual(prev.current, next)) return prev.current;
  prev.current = next;
  return next;
}
...
function useStableValue<T>(
  ...
  const next = selector();
  if (prev.current !== undefined && isEqual(prev.current, next)) return prev.current;
  prev.current = next;
  return next;
}
...
export function useLocationIds(): string[] {
  return useStableArray(
    () => useProjectStore.getState().locations.map((l) => l.id),
...
export function useLocationsForMap(): LocationMapMeta[] {
  return useStableArray(
    () =>
      useProjectStore.getState().locations.map((l) => ({
        id: l.id,
        name: l.name,
